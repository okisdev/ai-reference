...
3. **Schedule the monitor loop.** Schedule a recurring monitor on a short interval (for example about every 2 minutes) using the environment's native recurring automation mechanism; in Claude Code, use the available schedule or loop skill. Do not hardcode a non-portable scheduler. Each tick runs one idempotent Monitor cycle pass, so a missed or doubled tick is harmless.
...
5. **Merge, hold, or escalate.** When the Merge gate holds, merge per the gate's command, tear down the recurring automation (cancel or delete the scheduled routine; this does not apply to a self-paced loop), and stop (the only normal exit). A PR opened with `--draft` is a hold: keep polling but never `gh pr ready` or merge it until the user marks it ready. If the gate cannot clear (a gating check persistently red, an unmet required approval with no path to it, a DIRTY branch or a BEHIND branch whose update conflicts and needs a rebase or the original author, a gating reviewer unreachable, a fork head ship-pr cannot push to and therefore cannot auto-update when BEHIND, or `verify-pr` recommending close or redirect), tear down the automation, stop polling, and report the blocker. Cap iterations so the monitor never runs forever; treat UNKNOWN merge or check states as still computing and re-poll rather than escalate.
