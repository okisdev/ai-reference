...
4. **Run each monitor pass** following `references/monitor.md` for the full per-tick mechanics (gating-set discovery, check and reviewer classification, the reviewThreads read, merge-state handling, idempotency). Each pass is idempotent. Call `verify-pr` for the merge-worthiness recommendation only on the first pass after creation and after any substantial new commits, not every tick.
...
**See also**: `/create-branch`, `/commit-changes`, and `/create-pr` build the PR, `/verify-pr` judges whether it should merge, and `/verify-pr-comments` triages the review threads. The per-tick monitor mechanics live in `references/monitor.md`; the build sequence in `references/build-pr.md`.
