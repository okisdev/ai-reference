...
4. **Pick verification dimensions**, evaluated in this order:
   ...
   - **Correctness**, does the diff fix what the issue or description describes? Restate the claim falsifiably first (the condition, the observable, and what counts as fixed), then measure against that. When the fix targets runtime behavior (a crash, thrown error, wrong output, environment difference), prove it: `git worktree add ../verify-<num> && cd ../verify-<num> && gh pr checkout <num>`, run the repro or the PR's added tests against the real code to confirm the bug is gone, then remove the worktree. Prove both sides: the symptom appears on the merge base and disappears on the head under the same command and inputs; a symptom that never shows on the base means the PR fixes nothing observable, which is its own finding. For a UI-facing PR, render evidence composes `verify-ui`; for a CLI or TUI-facing PR, terminal evidence composes `verify-cli`; a diff that reads right can still render wrong. A diff that looks correct but does not resolve the bug is the failure this catches.
...
8. **Report** in this structure:
   ...
   - **Per-dimension verdict table**:
     ...
     | Correctness | OK / minor / fail / inconclusive | inconclusive = the measurement itself failed (no valid baseline, repro would not run); never soften it to OK |
