...
Reading the code is the floor, not the ceiling. Match verification depth to what the conclusion rests on; escalate past reading when it hinges on runtime behavior, an environment difference, a third-party API's real shape, or a flaw in the issue's own reasoning.

Distinct from siblings:
- **verify-pr**, judges whether a PR should merge (necessity, correctness, tests, conventions).
- **verify-pr-comments**, validates review comments left on a PR.

When verification surfaces a PR that already covers the issue, hand off to `/verify-pr` to judge it rather than re-deriving the fix here.
...
1. **Fetch issue**: Run `gh issue view $ISSUE_NUMBER --json title,body,labels,state,author,comments`. Extract the title, body, every technical claim, and the issue's own proposed conclusion or fix.
...
4. **Choose verification depth per claim**: escalate up this ladder per the intro's triggers; don't run the whole ladder on every claim, spend depth where the conclusion rests.
   ...
   - **L2 Reproduce**: for any claim about runtime behavior (a crash, a thrown error, wrong output, an environment-specific difference), run it, don't infer it. Write a minimal repro with the repo's own test runner (or a throwaway script), import the **real** code under test, observe the actual result, then delete the throwaway and never commit it. Use the same repro to test the issue's **proposed fix's** premise, not just the bug. For bugs that depend on real DOM, CSS, or focus behavior a unit runner can't reproduce, reach for Chrome DevTools MCP or Playwright; for pure logic, a unit test is enough. If a behavioral claim does not reproduce on current code, check `git log` / `git blame` for a fixing commit before concluding; 'already fixed in `<sha>`' is a distinct outcome from 'never valid', and the two warrant different responses to the issue.
...
- Read the actual source before judging; never guess from type names or file names alone.
- Flag inaccurate framing of standards even when the practical conclusion still holds; don't let a polished writeup smuggle in unverified citations.
