...
2. **Decompose into claims, and find the load-bearing one**: Sort each claim into the bucket that determines how it's verified:
   ...
   Then identify the **load-bearing claim**, the one the conclusion actually hinges on, and restate it falsifiably (the condition, the observable, and what would disprove it) before verifying. The issue's own framing and reasoning (its "because X, option Y is safe") are claims to test, not premises to accept; verify that one deepest.
...
4. **Choose verification depth per claim**: escalate up this ladder per the intro's triggers, not the whole ladder per claim.
   ...
   - **L2 Reproduce**: for any claim about runtime behavior (a crash, a thrown error, wrong output, an environment-specific difference), run it, don't infer it. Write a minimal repro with the repo's own test runner, import the **real** code under test, and observe the actual result. The discriminating symptom counts as reproduced only when it appears twice from a reset starting state; state inspection may confirm what was observed, never inject or force the symptom. Use the same repro to test the issue's **proposed fix's** premise, not just the bug. If a behavioral claim does not reproduce on current code, check `git log` / `git blame` for a fixing commit before concluding; 'already fixed in `<sha>`' is proven by showing the symptom on that commit's parent with the same repro, not by the commit message alone, and it is a distinct outcome from 'never valid'; the two warrant different responses to the issue.
   - **L3 Upstream**: for external, library, or runtime-API claims, compose `read-source`: it picks the rung by question type (installed code, the published npm artifact, or the upstream repository pinned to the release tag) and owns the tag and URL resolution mechanics. Check the version range the claim concerns, not just `main`, and don't trust the issue's characterization of a third-party API, or your own memory of it.
...
6. **Report** in this structure, in the same language as the user's input, noting version differences when a claim references a specific version:
   ...
   - **Verification table**, one row per claim. Note the method when it went beyond reading:
     ...
     |---|-------|--------|----------|
     | 1 | description | Confirmed / Refuted / Partial / Inconclusive / Already fixed in `<sha>` | `file:line`, or "confirmed by repro", or "checked against <lib> source v0.79 to v0.86". Brief explanation. Inconclusive = the check itself failed (repro would not run, environment blocked it); state what blocked it rather than rounding to Confirmed or Refuted. |
