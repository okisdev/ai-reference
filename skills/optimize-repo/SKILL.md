...
8. Sync docs: update every doc the change invalidates, prune stale ones, and keep them minimal, stripping AI tells on the way (puffery, chatbot phrases, decorative emojis, em-dash punctuation). With `--docs-only` over the whole repository, run the full inventory mode instead: list every README and doc, delete the obsolete ones outright, and rewrite the survivors into one coherent minimal structure.
